...
class FIRNpModel(BaseModel):
    def __init__(self, opt):
        super(FIRNpModel, self).__init__(opt)
        if opt['dist']:
        ...
        if self.is_train:
            ...
            # loss
            self.Reconstruction_forw = ReconstructionLoss(self.device,losstype=self.train_opt['pixel_criterion_forw'])
            self.Reconstruction_back = ReconstructionLoss(self.device,losstype=self.train_opt['pixel_criterion_back'])

            # feature loss
            if train_opt['feature_weight'] > 0:
                self.Reconstructionf = ReconstructionLoss(self.device,losstype=self.train_opt['feature_criterion'])
    ...
    def loss_backward(self, x, x_samples):
        ...
        # feature loss
        if self.l_fea_w > 0 or self.l_tex_w > 0:
